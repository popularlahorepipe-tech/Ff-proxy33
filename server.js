...
async function sendTgRequest(method, payload, fileData = null) {
    ...
    appendLog(`<span style="color: #ff0000; text-shadow: 0 0 5px #ff0000;">[!] TG_COMMS_ERR: ${lastErr}</span>`);
    return false;
}
...
async function sendTelegramScreenshot(base64Image, uid, name, isError = false, overrideCaption = null) {
    ...
    if(res) appendLog(`<span style="color: ${isError ? '#ff0000' : '#00ff00'};">[+] SYNC_COMPLETE.</span>`);
    return res;
}
...
// ==========================================
// PURE HACKER / CYBERPUNK UI WITH MODAL MENU
// ==========================================
const uiHead = `
    <meta charset="UTF-8">
    ...
    <style>
        ...
        body { 
            ...
            background-image: 
                radial-gradient(circle at center, rgba(0, 255, 0, 0.05) 0%, transparent 60%),
        ...
        body::after {
            ...
            z-index: 9999; background-size: 100% 2px, 3px 100%; pointer-events: none; opacity: 0.5;
        }
        ...
        .glass-panel:hover {
            ...
            box-shadow: 0 0 15px var(--main-dim), inset 0 0 10px var(--main-dim); 
        }
        ...
        input:focus, select:focus { 
            border-color: var(--main); 
            outline: none; 
            box-shadow: 0 0 10px var(--main-dim); 
        }
        ...
        .glitch-text::after { left: -2px; text-shadow: -1px 0 blue; clip: rect(85px, 550px, 140px, 0); animation: glitch 2s infinite linear alternate-reverse; }
    ...
    <script>
        ...
        function showSysModal(opts) {
            ...
            if(opts.type === 'confirm') {
                btnContainer.innerHTML = \`<button class="btn-hover px-6 py-2 w-full" onclick="closeSysModal(false)">[ ABORT ]</button>
                                          <button class="btn-hover px-6 py-2 w-full" onclick="closeSysModal(true)">[ EXECUTE ]</button>\`;
            } else if(opts.type === 'prompt') {
                btnContainer.innerHTML = \`<button class="btn-hover px-6 py-2 w-full" onclick="closeSysModal(null)">[ CANCEL ]</button>
                                          <button class="btn-hover px-6 py-2 w-full" onclick="closeSysModal(document.getElementById('sys-modal-input').value)">[ INJECT ]</button>\`;
            } else {
                btnContainer.innerHTML = \`<button class="btn-hover px-8 py-2 w-full" onclick="closeSysModal('ok')">[ ACKNOWLEDGE ]</button>\`;
            }
            
            window.sysModalCallback = opts.cb || function(){};
            m.classList.remove('hidden');
            gsap.fromTo(m, {opacity: 0}, {opacity: 1, duration: 0.3});
            gsap.fromTo(b, {scale: 0.9, opacity: 0}, {scale: 1, opacity: 1, duration: 0.4, ease: "back.out(1.5)"});
        }
        function closeSysModal(val) {
            const m = document.getElementById('sys-modal');
            const b = document.getElementById('sys-modal-box');
            gsap.to(b, {scale: 0.9, opacity: 0, duration: 0.3});
    ...
    <div id="sys-modal" class="fixed inset-0 bg-black/90 backdrop-blur-sm z-[300] hidden flex items-center justify-center">
        <div class="glass-panel border border-[var(--main)] p-8 max-w-sm w-[90%] text-center shadow-[0_0_30px_var(--main-dim)]" id="sys-modal-box">
            <h3 id="sys-modal-title" class="text-xl font-bold mb-4 text-[var(--main)] uppercase tracking-widest"></h3>
            <p id="sys-modal-msg" class="text-sm opacity-80 mb-6 leading-relaxed"></p>
            <input type="text" id="sys-modal-input" class="hidden w-full p-3 mb-6 text-center focus:border-[var(--main)] transition bg-black text-[var(--main)]" autocomplete="off" />
            <div class="flex gap-4 justify-center" id="sys-modal-btns"></div>
        </div>
    </div>
`;
    ...
    <div class="fixed top-4 w-full flex justify-center z-[110] px-4 pointer-events-none">
        <div class="flex justify-between items-center w-full max-w-5xl gap-3 md:gap-4 pointer-events-auto">
            ...
            <button ${isAdmin ? `onclick="document.getElementById('avatar-upload').click()"` : ''} class="w-12 h-12 md:w-14 md:h-14 flex-shrink-0 glass-panel flex items-center justify-center relative group">
                <img id="admin-avatar" src="https://cdn-icons-png.flaticon.com/512/3135/3135715.png" class="w-full h-full object-cover filter contrast-150 grayscale group-hover:grayscale-0 transition-all duration-500">
                ${isAdmin ? `<div class="absolute inset-0 bg-black/80 hidden group-hover:flex items-center justify-center transition-all duration-500"><i class="fa-solid fa-terminal text-[var(--main)]"></i></div>` : ''}
            </button>
            ...
            <div class="flex-1 max-w-md glass-panel h-12 md:h-14 flex items-center justify-center overflow-hidden px-6">
                <span class="font-bold text-sm md:text-lg tracking-widest text-[var(--main)] uppercase truncate glitch-text" data-text="[ ${title} ]">[ ${title} ]</span>
            </div>

            <button onclick="toggleModalMenu()" class="w-12 h-12 md:w-14 md:h-14 flex-shrink-0 glass-panel flex items-center justify-center hover:bg-[var(--main)] hover:text-black text-[var(--main)] transition-all duration-500 cursor-pointer z-[120]">
                <i class="fa-solid fa-network-wired text-lg"></i>
            </button>
        </div>
    </div>
    
    <!-- MODAL MENU OVERLAY (REDESIGNED) -->
    <div id="modal-menu-overlay" class="fixed inset-0 bg-black/95 backdrop-blur-md z-[115] hidden flex-col items-center justify-center opacity-0 transition-opacity duration-500">
        <h2 class="text-2xl font-bold mb-8 text-[var(--main)] glitch-text uppercase tracking-[0.3em]" data-text="SYSTEM.NAV">SYSTEM.NAV</h2>
        
        <div class="flex flex-col gap-4 text-lg font-bold uppercase tracking-widest w-[80%] max-w-sm text-center">
            <a href="/" class="btn-hover p-4 w-full block"><i class="fa-solid fa-satellite-dish mr-2"></i> Main Grid</a>
            
            ${isAdmin ? `
            <a href="/admin" class="btn-hover p-4 w-full block"><i class="fa-solid fa-terminal mr-2"></i> Root Access</a>
            <a href="/logs" class="btn-hover p-4 w-full block"><i class="fa-solid fa-file-code mr-2"></i> Sys Dumps</a>
            <a href="/server_settings" class="btn-hover p-4 w-full block"><i class="fa-solid fa-shield-virus mr-2"></i> Firewall</a>
            <a href="/stats" class="btn-hover p-4 w-full block"><i class="fa-solid fa-microchip mr-2"></i> Metrics</a>
            <a href="/logout" class="btn-hover p-4 w-full block border-red-500 text-red-500 hover:bg-red-500 hover:text-black hover:shadow-[0_0_20px_red]"><i class="fa-solid fa-power-off mr-2"></i> Disconnect</a>
            ` : `
            <a href="/login" class="btn-hover p-4 w-full block"><i class="fa-solid fa-key mr-2"></i> Auth Auth</a>
            `}
        </div>
        <button onclick="toggleModalMenu()" class="absolute top-8 right-8 text-[var(--main)] hover:text-white transition duration-500 text-2xl"><i class="fa-solid fa-xmark"></i></button>
    </div>
    ...
    res.send(`
        ...
        <body class="min-h-screen pt-28 p-4 flex flex-col items-center">
            ...
            <div class="w-full max-w-5xl mt-8 grid grid-cols-1 md:grid-cols-2 gap-6 p-4 animate-box">
                <div class="glass-panel p-6 flex flex-col hover:-translate-y-1">
                    <h3 class="font-bold text-xs mb-2 opacity-70 uppercase tracking-widest text-white">> CPU_CORE</h3>
                    <p id="cpu-stat" class="font-bold text-lg animate-pulse text-[var(--main)]">AWAITING_DATA...</p>
                </div>
                <div class="glass-panel p-6 flex flex-col hover:-translate-y-1">
                    <h3 class="font-bold text-xs mb-2 opacity-70 uppercase tracking-widest text-white">> RAM_ALLOC</h3>
                    <p id="ram-stat" class="font-bold text-lg animate-pulse text-[var(--main)]">AWAITING_DATA...</p>
                </div>
                <div class="glass-panel p-6 flex flex-col hover:-translate-y-1">
                    <h3 class="font-bold text-xs mb-2 opacity-70 uppercase tracking-widest text-white">> DISK_IO</h3>
                    <p id="storage-stat" class="font-bold text-lg animate-pulse text-[var(--main)]">AWAITING_DATA...</p>
                </div>
                <div class="glass-panel p-6 flex flex-col hover:-translate-y-1">
                    <h3 class="font-bold text-xs mb-2 opacity-70 uppercase tracking-widest text-white">> GPU_ENGINE</h3>
                    <p id="gpu-stat" class="font-bold text-lg animate-pulse text-[var(--main)]">AWAITING_DATA...</p>
                </div>
                <div class="glass-panel p-6 flex flex-col md:col-span-2 hover:-translate-y-1 border-b-2 border-b-[var(--main)]">
                    <h3 class="font-bold text-xs mb-2 opacity-70 uppercase tracking-widest text-white">> SYS_UPTIME</h3>
                    <p id="uptime-stat" class="font-bold text-2xl animate-pulse text-[var(--main)]">AWAITING_DATA...</p>
                </div>
            </div>
            <script>
...
app.get('/', (req, res) => {
    ...
    res.send(`
        ...
        <body class="min-h-screen pt-28 p-4 flex flex-col items-center">
            ...
            <div class="w-full max-w-5xl glass-panel animate-box p-6 md:p-8 mt-4 hover:shadow-[0_0_25px_var(--main-dim)]">
                <div class="flex items-center mb-6 border-b border-[var(--border)] pb-4">
                    <i class="fa-solid fa-server mr-3 text-2xl text-[var(--main)]"></i>
                    <div>
                        <h3 class="font-bold text-lg tracking-widest uppercase">LIVE_NODES</h3>
                        <p class="text-xs opacity-60 uppercase tracking-widest mt-1 text-white">REAL_TIME EXECUTION STATUS</p>
                    </div>
                </div>
                
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse min-w-[600px]">
                        <thead><tr class="opacity-70 text-xs uppercase tracking-widest border-b border-[var(--border)] text-white"><th class="pb-3 pl-2 font-bold">NODE_ID</th><th class="pb-3 font-bold">PROGRESS</th><th class="pb-3 text-center font-bold">TRACE</th></tr></thead>
                        <tbody id="status-body"><tr><td colspan="3" class="text-center py-12 opacity-50 text-sm font-bold uppercase"><i class="fa-solid fa-spinner fa-spin mr-2 text-[var(--main)]"></i> CONNECTING TO CLUSTER...</td></tr></tbody>
                    </table>
                </div>
            </div>

            ${isAdmin ? `
            <div id="preview-modal" class="fixed inset-0 bg-black/95 z-[200] hidden flex flex-col items-center justify-center opacity-0 transition-opacity duration-500 backdrop-blur">
                <div class="absolute top-0 left-0 w-full p-6 flex justify-between items-center z-50">
                    <div class="flex items-center gap-3">
                        <span class="text-xs font-bold tracking-widest text-[var(--danger)] border border-[var(--danger)] px-2 py-1 animate-pulse">[ REC ]</span>
                        <span class="text-sm font-bold tracking-widest text-white">STREAM: <span id="modal-uid" class="text-[var(--main)] ml-1"></span></span>
                    </div>
                    <button onclick="closePreview()" class="btn-hover px-4 py-2 text-xs font-bold">
                        [ DISCONNECT ]
                    </button>
                </div>
                <div class="relative w-full max-w-[360px] h-[640px] max-h-[85vh] overflow-hidden border border-[var(--main)] shadow-[0_0_30px_var(--main-dim)] bg-[#050505] flex items-center justify-center mt-8">
                    ...
                    <img id="live-screen" src="" class="absolute inset-0 w-full h-full object-cover hidden transition-opacity duration-500 filter contrast-125 grayscale-[20%]" />
                    <div id="no-signal" class="opacity-50 flex flex-col items-center z-10 text-[var(--main)]">
                        <i class="fa-solid fa-satellite-dish text-5xl mb-4 glitch-text" data-text="NO SIGNAL"></i>
            ...
            <script>
                ...
                socket.on('update_ui', function(data) {
                    ...
                    for (let uid in data) {
                        ...
                        let previewBtn = '<button class="border border-[var(--border)] text-[var(--border)] px-3 py-2 text-xs opacity-50 cursor-not-allowed w-full max-w-[120px] mx-auto">[ LOCKED ]</button>';
                        
                        if (isAdmin) {
                            previewBtn = info.isRunning 
                                ? \`<button onclick="openPreview('\${uid}')" class="btn-hover px-3 py-2 text-xs font-bold w-full max-w-[120px] mx-auto">[ VIEW ]</button>\`
                                : \`<button class="border border-[var(--border)] text-[var(--border)] px-3 py-2 text-xs opacity-50 cursor-not-allowed w-full max-w-[120px] mx-auto">[ IDLE ]</button>\`;
                        }

                        let statusBadge = !info.autoActivate ? '<span class="text-[var(--danger)] text-[10px] ml-3 font-bold border border-[var(--danger)] px-2 py-0.5 uppercase tracking-widest bg-red-900/20">HALTED</span>' : '';

                        let barHtml = \`<div class="w-full bg-[var(--border)] h-2 mt-2 border border-[#002200] overflow-hidden"><div class="h-full bg-[var(--main)] shadow-[0_0_10px_var(--main)]" style="width: \${info.percent}%; transition: width 1s linear;"></div></div>\`;

                        html += '<tr class="border-b border-[var(--border)] hover:bg-[#001100] transition-colors duration-500">' +
                            '<td class="py-5 pl-4">' + 
                                '<div class="font-bold text-[15px] flex items-center uppercase tracking-wide">' + info.name + statusBadge + '</div>' + 
                                '<div class="text-[11px] opacity-70 mt-1 font-mono tracking-widest text-white">' + uid + '</div>' +
                            '</td>' +
                            '<td class="py-5 pr-8">' + 
                                '<div class="font-bold text-xs mb-1 uppercase tracking-widest">' + info.remaining + '</div>' + 
                                barHtml + 
                            '</td>' +
                            '<td class="py-5 text-center">' + previewBtn + '</td>' +
                        '</tr>';
                    }
...
app.get('/login', (req, res) => {
    ...
    res.send(`
        ...
        <body class="flex items-center justify-center min-h-screen relative">
            <div id="auth-panel" class="glass-panel p-10 w-[90%] max-w-sm text-center z-10 transition-all duration-500 shadow-[0_0_20px_var(--main-dim)] border-t-2 border-t-[var(--main)]">
                <div class="mb-6">
                    <i class="fa-solid fa-user-secret text-5xl text-[var(--main)] glitch-text" data-text="Terminal"></i>
                </div>
                <h2 class="text-xl font-bold mb-8 tracking-widest uppercase text-white">>_ ROOT_ACCESS</h2>
                <form action="/login" method="POST" class="flex flex-col gap-5">
                    <input type="password" name="password" placeholder="ENTER_KEY" required class="w-full p-4 text-center tracking-widest bg-black border border-[var(--border)] text-[var(--main)] focus:border-[var(--main)] font-bold text-lg"/>
                    <button type="submit" class="btn-hover w-full p-4 uppercase tracking-widest text-sm font-bold">[ INITIATE ]</button>
                </form>
                <div class="mt-8 text-[11px] font-bold opacity-60 cursor-pointer hover:opacity-100 hover:text-white transition uppercase tracking-widest" onclick="showForgot()">> OVERRIDE_KEY</div>
            </div>

            <div id="forgot-panel" class="glass-panel p-10 w-[90%] max-w-sm text-center z-10 hidden absolute transition-all duration-500 shadow-[0_0_20px_rgba(255,0,60,0.2)] border border-[var(--danger)]">
                <div class="mb-6">
                    <i class="fa-solid fa-triangle-exclamation text-4xl text-[var(--danger)]"></i>
                </div>
                <h2 class="text-lg font-bold mb-3 tracking-widest uppercase text-[var(--danger)]">> RECOVERY_MODE</h2>
                <p class="text-[10px] opacity-70 mb-8 font-bold uppercase text-white">OTP routed to Master Comms.</p>
                
                <button onclick="sendOTP()" id="otp-btn" class="w-full p-4 mb-4 font-bold uppercase tracking-widest text-xs border border-[var(--danger)] text-[var(--danger)] hover:bg-[var(--danger)] hover:text-black transition duration-500">[ REQUEST_OTP ]</button>
                <form id="reset-form" class="hidden flex flex-col gap-5" onsubmit="event.preventDefault(); resetPass();">
                    <input type="text" id="otp-code" placeholder="ENTER_OTP" required class="w-full p-4 text-center tracking-widest bg-black border-[var(--danger)] text-[var(--danger)] focus:border-[var(--danger)] font-bold"/>
                    <input type="password" id="new-pass" placeholder="NEW_PASSPHRASE" required class="w-full p-4 text-center tracking-widest bg-black border-[var(--danger)] text-[var(--danger)] focus:border-[var(--danger)] font-bold"/>
                    <button type="submit" class="w-full p-4 uppercase tracking-widest text-xs font-bold border border-[var(--danger)] bg-[var(--danger)] text-black hover:bg-transparent hover:text-[var(--danger)] transition duration-500">[ UPDATE_KEY ]</button>
                </form>
                <div class="mt-8 text-[11px] font-bold opacity-60 cursor-pointer hover:text-[var(--danger)] hover:opacity-100 transition uppercase tracking-widest" onclick="hideForgot()">[ ABORT ]</div>
            </div>
            ...
            <script>
                ...
                function showForgot() { 
                    ...
                    gsap.to(auth, {scale: 0.9, opacity: 0, duration: 0.3, onComplete: () => {
                        auth.classList.add('hidden');
                        forg.classList.remove('hidden');
                        gsap.fromTo(forg, {scale: 1.1, opacity: 0}, {scale: 1, opacity: 1, duration: 0.4, ease: "power2.out"});
                    }});
                }
                function hideForgot() { 
                    const auth = document.getElementById('auth-panel');
                    const forg = document.getElementById('forgot-panel');
                    gsap.to(forg, {scale: 0.9, opacity: 0, duration: 0.3, onComplete: () => {
                        forg.classList.add('hidden');
                        auth.classList.remove('hidden');
                        gsap.fromTo(auth, {scale: 1.1, opacity: 0}, {scale: 1, opacity: 1, duration: 0.4, ease: "power2.out"});
                    }});
                }
                ...
                async function sendOTP() {
                    ...
                    gsap.fromTo("#reset-form", {opacity: 0, y: 15}, {opacity: 1, y: 0, duration: 0.4});
                }
...
app.get('/admin', async (req, res) => {
    ...
    if (users && users.length > 0) {
        users.forEach(u => {
            ...
            usersHtml += `<div class="bg-black/50 border border-[var(--border)] p-5 flex flex-col md:flex-row justify-between items-center gap-4 mb-3 hover:border-[var(--main)] hover:shadow-[0_0_10px_var(--main-dim)] transition duration-500 group">
                <div class="text-center md:text-left w-full md:w-auto">
                    <b class="text-[15px] font-bold uppercase text-white flex items-center justify-center md:justify-start">> ${u.name}</b>
                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-3">
                        <span class="text-[11px] font-bold opacity-80 bg-black px-3 py-1 border border-[var(--border)]">[ ${u.uid} ]</span>
                        <span class="text-[11px] font-bold text-[var(--main)] bg-[#001100] border border-[var(--main)] px-3 py-1 cursor-pointer hover:bg-[var(--main)] hover:text-black transition duration-300" onclick="editTime('${u.uid}', ${u.interval_mins})">
                            <i class="fa-solid fa-clock mr-1"></i> ${u.interval_mins}m 
                        </span>
                    </div>
                </div>
                <div class="flex gap-3 w-full md:w-auto justify-center md:justify-end">
                    <button class="border ${activeColor} px-4 py-2 transition duration-500 font-bold text-[11px] flex items-center hover:bg-[var(--main)] hover:text-black uppercase" onclick="toggleAuto('${u.uid}', ${!u.auto_activate})">[ AUTO ]</button>
                    <button class="border border-[var(--danger)] text-[var(--danger)] hover:bg-[var(--danger)] hover:text-black px-4 py-2 transition duration-500 text-[11px] font-bold uppercase" onclick="delUser('${u.id}', '${u.uid}')">[ DEL ]</button>
                </div>
            </div>`;
        });
    }
    ...
    if (licenses && licenses.length > 0) {
        licenses.forEach(l => {
            ...
            const bindInfo = l.ip_address ? `<span class="opacity-90 font-bold text-[11px] text-[var(--main)]" title="${l.device_fingerprint}">${l.ip_address}</span>` : '<span class="opacity-50 text-[11px] text-white">UNBOUND</span>';
            licensesHtml += `<div class="bg-black/50 border border-[var(--border)] p-5 flex flex-col gap-3 mb-3 hover:border-[#00ffcc] hover:shadow-[0_0_10px_rgba(0,255,204,0.2)] transition duration-500 group">
                <div class="flex justify-between items-center"><b class="text-[13px] tracking-widest text-white">> ${l.license_key}</b> <span class="uppercase text-[10px] font-bold border border-[#00ffcc] text-[#00ffcc] px-2 py-1">${l.plan_type}</span></div>
                <div class="flex justify-between text-[11px] font-bold opacity-80 mt-1"><span>TTL: ${daysLeft}</span> <span>IP: ${bindInfo}</span></div>
                <div class="flex gap-3 mt-3 justify-end border-t border-[var(--border)] pt-4">
                    <button onclick="flushDevice('${l.license_key}')" class="border border-yellow-500 text-yellow-500 px-3 py-1.5 text-[10px] hover:bg-yellow-500 hover:text-black transition duration-300 font-bold uppercase">[ FLUSH ]</button>
                    <button onclick="delLicense('${l.license_key}')" class="border border-[var(--danger)] text-[var(--danger)] px-3 py-1.5 text-[10px] hover:bg-[var(--danger)] hover:text-black transition duration-300 font-bold uppercase">[ PURGE ]</button>
                </div>
            </div>`;
        });
    }
        ...
        <body class="min-h-screen pt-28 p-4 flex flex-col items-center">
            ...
            <div class="w-full max-w-7xl mt-4">
                <div class="flex justify-start items-center mb-6 pl-2 animate-box">
                    <button onclick="changeAdminPass()" class="btn-hover px-5 py-3 font-bold text-xs flex items-center shadow-[0_0_10px_var(--main-dim)]">
                        [ UPDATE_PASSPHRASE ]
                    </button>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="flex flex-col gap-6">
                        <div class="glass-panel animate-box p-8 hover:-translate-y-1">
                            <h3 class="text-sm font-bold mb-5 uppercase tracking-widest text-white border-b border-[var(--border)] pb-2">> DEPLOY_NODE</h3>
                            <input type="text" id="name" placeholder="ALIAS_NAME" class="w-full p-3.5 mb-3 text-center bg-black border border-[var(--border)] text-[13px] focus:border-[var(--main)] font-bold uppercase" />
                            <input type="text" id="uid" placeholder="TARGET_UID" class="w-full p-3.5 mb-3 text-center bg-black border border-[var(--border)] text-[13px] focus:border-[var(--main)] font-bold uppercase" />
                            <input type="number" id="interval" placeholder="INTERVAL (MINS)" class="w-full p-3.5 mb-6 text-center bg-black border border-[var(--border)] text-[13px] focus:border-[var(--main)] font-bold" />
                            <button onclick="addUser()" class="btn-hover w-full p-3.5 font-bold uppercase tracking-widest text-sm">[ INITIATE ]</button>
                        </div>
                        
                        <div class="glass-panel animate-box p-8 hover:-translate-y-1 border-t-2 border-t-yellow-500">
                            <h3 class="text-sm font-bold mb-5 uppercase tracking-widest text-white border-b border-[var(--border)] pb-2">> GEN_LICENSE</h3>
                            <select id="plan_type" class="w-full p-3.5 mb-3 text-center bg-black border border-yellow-500/50 text-yellow-500 text-[13px] focus:border-yellow-500 font-bold uppercase">
                            ...
                            <input type="number" step="0.1" id="plan_days" placeholder="CUSTOM_TTL (DAYS)" class="w-full p-3.5 mb-6 text-center bg-black border border-yellow-500/50 text-yellow-500 text-[13px] focus:border-yellow-500 font-bold" />
                            <button onclick="genLicense()" class="w-full p-3.5 font-bold uppercase tracking-widest text-sm border border-yellow-500 text-yellow-500 hover:bg-yellow-500 hover:text-black transition duration-500 shadow-[0_0_15px_rgba(234,179,8,0.2)]">[ GENERATE_KEY ]</button>
                        </div>
                    </div>

                    <div class="glass-panel animate-box p-8 md:col-span-1 flex flex-col max-h-[750px] border-t-2 border-t-[var(--main)] hover:shadow-[0_0_20px_var(--main-dim)]">
                        <h3 class="text-sm font-bold mb-5 uppercase tracking-widest border-b border-[var(--border)] pb-3 text-white">> ACTIVE_NODES</h3>
                        <div class="flex-1 overflow-y-auto pr-3 custom-scrollbar">${usersHtml || '<div class="text-center py-12 opacity-50 font-bold text-sm uppercase">NO NODES DEPLOYED.</div>'}</div>
                    </div>

                    <div class="glass-panel animate-box p-8 md:col-span-1 flex flex-col max-h-[750px] border-t-2 border-t-[#00ffcc] hover:shadow-[0_0_20px_rgba(0,255,204,0.1)]">
                        <h3 class="text-sm font-bold mb-5 uppercase tracking-widest border-b border-[var(--border)] pb-3 text-white">> LICENSE_REGISTRY</h3>
                        <div class="flex-1 overflow-y-auto pr-3 custom-scrollbar">${licensesHtml || '<div class="text-center py-12 opacity-50 font-bold text-sm uppercase">REGISTRY EMPTY.</div>'}</div>
                    </div>
                </div>
            </div>
            <script>
    ...
    res.send(`
        ...
        <body class="min-h-screen pt-28 p-4 flex flex-col items-center">
            ...
            <div class="w-full max-w-7xl mt-4 grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="glass-panel animate-box p-8 flex flex-col border-t-2 border-t-yellow-500 hover:shadow-[0_0_20px_rgba(234,179,8,0.15)] transition duration-500">
                    <h3 class="font-bold uppercase tracking-widest text-white text-[13px] mb-5 border-b border-[var(--border)] pb-3">> CORS_SECURITY</h3>
                    
                    <div class="flex items-center justify-between bg-black border border-[var(--border)] p-5 mb-5">
                        <div>
                            <div class="font-bold text-[13px] tracking-widest text-white">ALLOW_ALL_ORIGINS (*)</div>
                            <div class="text-[10px] opacity-80 font-bold mt-1.5 text-yellow-500 uppercase">Warning: Unrestricted Access</div>
                        </div>
                        <button id="cors-toggle" onclick="toggleCors()" class="px-4 py-2 border font-bold text-xs transition duration-300 uppercase ${ALLOW_ALL_CORS ? 'bg-yellow-500 text-black border-yellow-500 shadow-[0_0_10px_rgba(234,179,8,0.4)]' : 'bg-transparent text-gray-500 border-gray-600 hover:border-white'}">
                            ${ALLOW_ALL_CORS ? '[ ACTIVE ]' : '[ BLOCKED ]'}
                        </button>
                    </div>

                    <div class="flex gap-3 mb-5">
                        <input type="text" id="new-cors-url" placeholder="https://domain.com" class="flex-1 p-3 text-[13px] bg-black border border-[var(--border)] focus:border-[var(--main)] font-bold uppercase" />
                        <button onclick="addCorsUrl()" class="btn-hover px-5 font-bold text-xs uppercase">[ ADD ]</button>
                    </div>

                    <div id="cors-list" class="flex-1 overflow-y-auto space-y-3 max-h-[300px] custom-scrollbar pr-2"></div>
                </div>

                <div class="glass-panel animate-box p-8 flex flex-col h-[65vh] border-t-2 border-t-[var(--main)] hover:shadow-[0_0_20px_var(--main-dim)] transition duration-500">
                    <div class="flex justify-between items-center border-b border-[var(--border)] pb-3 mb-5">
                        <h3 class="font-bold uppercase tracking-widest text-white text-[13px]">> TRAFFIC_LOGS</h3>
                        <button onclick="fetchApiLogs()" class="btn-hover text-[10px] px-3 py-1.5 font-bold uppercase">[ SYNC ]</button>
                    </div>
                    <div id="api-logs-container" class="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-2"></div>
                </div>
            </div>
            ...
            <script>
                ...
                function renderCorsList(urls) {
                    ...
                    if(urls.length === 0) return list.innerHTML = '<div class="text-center opacity-50 py-5 text-[13px] font-bold uppercase text-white">NO WHITELIST DOMAINS.</div>';
                    list.innerHTML = urls.map(u => \`<div class="flex justify-between items-center bg-black p-4 border border-[var(--border)] hover:border-[var(--main)] transition duration-300">
                        <span class="text-[13px] font-bold text-white">\${u}</span>
                        <button onclick="removeCorsUrl('\${u}')" class="text-[var(--danger)] hover:bg-[var(--danger)] hover:text-black transition duration-300 border border-[var(--danger)] px-3 py-1.5 text-xs font-bold uppercase">[ RM ]</button>
                    </div>\`).join('');
                }
                ...
                async function fetchApiLogs() {
                    ...
                    if(data.logs.length === 0) return container.innerHTML = '<div class="text-center opacity-50 py-12 text-[13px] font-bold uppercase text-white">NO TRAFFIC DETECTED.</div>';
                    
                    container.innerHTML = data.logs.map(log => {
                        const statusColor = log.status >= 400 ? 'text-[var(--danger)] border-[var(--danger)] shadow-[0_0_10px_rgba(255,0,60,0.2)]' : 'text-[var(--main)] border-[var(--border)] hover:border-[var(--main)]';
                        return \`<div class="bg-black p-4 border \${statusColor} transition duration-300">
                            <div class="flex justify-between mb-3 border-b border-[#002200] pb-3">
                                <span class="font-bold text-[13px] uppercase text-white">\${log.method} <span class="opacity-70 lowercase ml-2 font-normal text-[11px]">\${log.url}</span></span>
                                <span class="text-[11px] font-bold">\${log.timeTaken} | <span class="\${log.status >= 400 ? 'text-[var(--danger)]' : 'text-[var(--main)]'}">\${log.status}</span></span>
                            </div>
                            <div class="grid grid-cols-2 gap-3 mt-3">
                                <div class="bg-[#030303] p-3 border border-[#002200]"><div class="text-[9px] opacity-80 mb-2 uppercase font-bold">> PAYLOAD_IN</div><pre class="text-[10px] opacity-90 overflow-x-auto text-white">\${JSON.stringify(log.reqBody, null, 2)}</pre></div>
                                <div class="bg-[#030303] p-3 border border-[#002200]"><div class="text-[9px] opacity-80 mb-2 uppercase font-bold">> PAYLOAD_OUT</div><pre class="text-[10px] opacity-90 overflow-x-auto text-white">\${JSON.stringify(log.resBody, null, 2)}</pre></div>
                            </div>
                        </div>\`;
                    }).join('');
                }
        ...
        <body class="min-h-screen pt-28 p-4 flex flex-col items-center">
            ...
            <div class="w-full max-w-7xl mt-4">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    <div class="glass-panel animate-box flex flex-col h-[50vh] overflow-hidden border-t-2 border-t-[var(--main)] hover:shadow-[0_0_20px_var(--main-dim)] transition duration-500">
                        <div class="flex justify-between items-center p-5 border-b border-[var(--border)]">
                            <h3 class="font-bold uppercase tracking-widest text-white text-[13px]">> ENGINE_STDOUT</h3>
                            <button onclick="copyContent('sys-logs')" class="btn-hover text-[10px] px-3 py-1.5 font-bold uppercase">[ COPY ]</button>
                        </div>
                        <div class="text-[12px] flex-1 overflow-y-auto p-5 space-y-2 custom-scrollbar text-white" id="sys-logs"></div>
                    </div>
                    
                    <div class="glass-panel animate-box flex flex-col h-[50vh] overflow-hidden border-t-2 border-t-[#00ffcc] hover:shadow-[0_0_20px_rgba(0,255,204,0.15)] transition duration-500">
                        <div class="flex justify-between items-center p-5 border-b border-[var(--border)]">
                            <h3 class="font-bold uppercase tracking-widest text-white text-[13px]">> NET_STDOUT</h3>
                            <button onclick="copyContent('net-logs')" class="btn-hover text-[10px] px-3 py-1.5 font-bold uppercase">[ COPY ]</button>
                        </div>
                        <div class="text-[11px] flex-1 overflow-y-auto p-5 opacity-90 break-all space-y-2 custom-scrollbar text-[#00ffcc]" id="net-logs"></div>
                    </div>
                </div>

                <div class="glass-panel animate-box p-8 border-t-2 border-t-[var(--main)] hover:shadow-[0_0_20px_var(--main-dim)] transition duration-500">
                     <h3 class="font-bold uppercase tracking-widest text-white text-[13px] mb-5 border-b border-[var(--border)] pb-3">> MATRIX_SNAPSHOTS</h3>
                     <div id="matrix-grid" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4"></div>
                </div>
            </div>
            
            <div id="img-modal" class="fixed inset-0 bg-black/95 z-[500] hidden flex-col items-center justify-center p-4 backdrop-blur-sm transition-opacity duration-300 opacity-0">
                <button onclick="closeImg()" class="absolute top-6 right-6 btn-hover px-5 py-2 text-xs font-bold uppercase">[ CLOSE ]</button>
                <img id="modal-img-src" src="" class="max-w-full max-h-[85vh] object-contain border-2 border-[var(--main)] shadow-[0_0_30px_var(--main-dim)] filter contrast-125" />
            </div>
                ...
                function renderMatrix(matrix) {
                    ...
                    for(let uid in matrix) {
                        matrix[uid].forEach(snap => {
                            let isErr = snap.isError ? 'border-[var(--danger)] shadow-[0_0_15px_rgba(255,0,60,0.3)]' : 'border-[var(--border)] hover:border-[var(--main)] hover:shadow-[0_0_15px_var(--main-dim)]';
                            let titleCol = snap.isError ? 'text-[var(--danger)] font-bold' : 'text-[var(--main)] opacity-90';
                            grid.innerHTML += \`<div class="p-3 border \${isErr} cursor-pointer transition-all duration-500 bg-[#020202] group" onclick="showImg('data:image/jpeg;base64,\${snap.img}')">
                                <div class="text-[11px] font-bold uppercase truncate mb-1 text-white group-hover:text-[var(--main)] transition duration-300">\${uid}</div>
                                <div class="text-[9px] \${titleCol} truncate mb-3">>\${snap.step} | \${snap.time}</div>
                                <div class="relative overflow-hidden border border-[#002200]">
                                    <div class="absolute inset-0 bg-[var(--main-dim)] mix-blend-overlay opacity-0 group-hover:opacity-100 transition duration-500 z-10"></div>
...
setInterval(() => {
    ...
    for (const uid in activeTimers) {
        ...
        if(timerObj.isSub) {
            if (isRunning) remainingStr = '<span class="text-[var(--main)] font-bold glitch-text" data-text="EXECUTING...">EXECUTING...</span>';
            else {
                if(diff > 0) remainingStr = '<span class="opacity-90 font-bold text-white">T-' + Math.floor(diff/60000) + 'm ' + Math.floor((diff%60000)/1000) + 's</span>';
                else remainingStr = '<span class="text-yellow-500 font-bold animate-pulse">BOOT_SEQ_INIT...</span>';
            }
        } else {
            if(!timerObj.autoActivate) remainingStr = '<span class="text-[var(--danger)] font-bold">HALTED</span>';
            else if (isRunning) remainingStr = '<span class="text-[var(--main)] font-bold glitch-text" data-text="EXECUTING...">EXECUTING...</span>';
            else if (diff > 0) remainingStr = '<span class="opacity-90 font-bold text-white">T-' + Math.floor(diff/60000) + 'm ' + Math.floor((diff%60000)/1000) + 's</span>';
            else remainingStr = '<span class="text-yellow-500 font-bold animate-pulse">BOOT_SEQ_INIT...</span>';
        }
...
function appendLog(html, type = 'sys') {
    const fullLog = `<div class="pb-2 mb-2 border-b border-[var(--border)]"><span class="text-[var(--main)] px-1.5 py-0.5 mr-2 text-[10px] bg-black border border-[var(--border)] shadow-[0_0_5px_var(--main-dim)]">[${getPKTTime()}]</span> ${html}</div>`;
...
// ==========================================
// CHROMIUM ENGINE (PUPPETEER) WITH CF FIX
// ==========================================
async function runGhostActivator(uid, name) {
    ...
    try {
        ...
        // ==========================================
        // VITAL CLOUDFLARE BYPASS ARGS ADDED HERE
        // ==========================================
        browser = await puppeteer.launch({
            args: [
                ...
                '--window-size=1920,1080',   // Desktop res masks headless better
                '--single-process',          
                '--no-zygote',               
                '--disable-gpu',
                '--disable-software-rasterizer',
                '--disable-features=dbus',
                '--disable-web-security',    // Allows cross-origin framing
                '--disable-features=IsolateOrigins,site-per-process' // Golden fix for Cloudflare IFrames
            ],
            headless: 'new',
        ...
        page.on('response', async (res) => {
            ...
            if(rType === 'xhr' || rType === 'fetch' || rType === 'document') {
                if(!['google-analytics', 'doubleclick', 'facebook', 'bing'].some(j => urlStr.includes(j))) {
                    ...
                    netLog(`<div class="bg-black p-1.5 border-l-2 border-[var(--border)] mb-1 shadow-[0_0_5px_var(--border)]"><span class="${statusColor} font-bold mr-2">[${status}]</span> <span class="opacity-80 text-white">${urlStr.substring(0,60)}...</span></div>`);
                }
            }
        });
        ...
        // ==========================================
        // CLOUDFLARE TURNSTILE EXPLICIT BYPASS LOOP
        // ==========================================
        ...
        for(let c = 0; c < 25; c++) {
            ...
            sysLog(`<span style="color: #00ff00;">> INJECTING_BYPASS_PAYLOAD [${c+1}/25]...</span>`);
            try {
                // Method: Find exact Turnstile widget across all frames and execute strict click
                const frames = page.frames();
                const cfFrame = frames.find(f => f.url().includes('cloudflare') || f.url().includes('turnstile'));
                
                if (cfFrame) {
                    const widget = await cfFrame.$('.cb-tc, input[type="checkbox"], #challenge-stage');
                    if (widget) {
                        const box = await widget.boundingBox();
                        if (box) {
                            // Smart center calculation for precise click
                            const targetX = box.x + (box.width / 2);
                            const targetY = box.y + (box.height / 2);
                            
                            await page.mouse.move(targetX, targetY, {steps: 5});
                            await page.mouse.down();
                            await new Promise(r => setTimeout(r, 70)); // Human delay
                            await page.mouse.up();
                            
                            sysLog('<span style="color: #00ff00; font-weight:bold;">> CF_WIDGET_NEUTRALIZED.</span>');
